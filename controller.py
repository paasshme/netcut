...
def controller(request):
    if request[0] == "nmap_scan":
            ...
            if len(request) == 1:
                return(json.dumps(nc.nmap_scan()))
            elif len(request) > 1:
                return(json.dumps(nc.nmap_scan(request[1])))

    elif request[0] == "arp_scan":
        
        if len(request) == 1:
            return(json.dumps(nc.arp_scan()))
            print(json.dumps(nc.arp_scan()))
        elif len(request) > 1:
            return(json.dumps(nc.arp_scan(request[1])))
        ...
        if len(request) > 2 and spoofed.count(request[1]) < 1 :
            ...
            return (json.dumps("Error : number of argument supplied is not correct"))
        ...
        if len(request) > 2:
            ...
            return(json.dumps("Error: number of argument supplied is not equal to 2"))
